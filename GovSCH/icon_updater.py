...
def update_cmmc_icons():
    ...
    while has_more:
        ...
        for page in response["results"]:
            page_id = page["id"]
            # Extract Domain from the 'Select' property
            domain_prop = page["properties"].get("Domain", {}).get("select")
            
            if domain_prop:
                domain_name = domain_name = domain_prop["name"]
    ...
    print(f"\n✅ Mission Complete. {count} icons applied to the framework.")
