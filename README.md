...
Threat Intelligence → Multi-Framework GRC Intelligence — Automated
...
## Architecture
DARKSWORD runs two independent ingestion chains. Both terminate in Notion databases that serve as the durable system of record for CPE, framework mapping, and adversary tracking.
...
Threat Actor Registry	darknetdiaries_ingest.py, crowdstrike_ingest.py	Auto-created from ingest	Actor stubs with classification, attribution confidence, evidence governance, and PSOA taxonomy
TTP Registry	darknetdiaries_ingest.py, crowdstrike_ingest.py	MITRE ATT&CK	ATT&CK technique tracking with confidence scoring and two-way framework bridge
Malware DB	darknetdiaries_ingest.py, crowdstrike_ingest.py	Ingest-derived	Malware family tracking with actor linkage, platform/capability fields, and CMMC/NIST impact
Master Frameworks	shared	CMMC 2.0 / NIST 800-171 / NIST CSF 2.0 / ISO 27001 / PCI DSS / SOC 2 / NY DFS 500 / Reg S-P / SEC Cyber Rules 2023 (474 controls)	Control mapping source of truth. Source Confidence on all rows; Framework Function/Domain backfilled; two-way TTP bridge live.
...
## CMMC Cache
The script queries the Master Frameworks database at launch and builds an in-memory cache indexed by NIST 800-171 Ref. The cache loads only CMMC-related rows — 133 rows total (CMMC 2.0, NIST 800-171 Rev 3, NIST AI RMF), reporting 111 distinct NIST refs. Lower than the row count because CMMC L1/L2 pairs citing the same underlying control (e.g. AC.L1-3.1.1 and AC.L2-3.1.1) share one ref key; resolve_control() disambiguates by maturity level. One row (MAP.1.5, NIST AI RMF) has no NIST 800-171 ref and is correctly excluded from the cache. Master Frameworks DB total is 474 rows across 8 frameworks — the cache indexes the CMMC subset only.
...
- [x] load_cmmc_cache() root cause closed — SR.L2-3.15.2 annotated-ref normalization fix applied (2026-09-15)
- [x] SR/SA bad row cleanup — 12 phantom CMMC 2.0 rows deleted, cache baseline 133 rows / 111 refs (2026-09-15)
- [x] Strategic scope pivot — CMMC-centric → SEC/financial sector multi-framework (2026-09-16)
- [x] Multi-framework pivot — 474 controls across 8 frameworks in Master Frameworks DB (2026-09-19)
- [x] Master Frameworks facelift — Source Confidence, Framework Function/Domain, two-way TTP bridge (2026-09-19)
- [x] Threat Actor Registry facelift — evidence governance, attribution confidence, PSOA taxonomy, 11 views (2026-09-19)
- [x] TTP Registry facelift — confidence scoring, legacy mapping, framework bridge (2026-09-19)
- [x] Malware DB facelift — review controls, actor/framework linkage, 9 views (2026-09-19)
- [ ] CrowdStrike incremental monitoring — title classifier for new adversary-profile episodes
- [ ] TTP ↔ Framework mapping population — ATT&CK-to-control mappings (CTID dataset for CMMC rows; per-engagement for SEC frameworks)
- [ ] Cross-DB review surface — unified triage view across Threat Actor Registry, TTP Registry, Malware DB, Master Frameworks
...
- `--dry-run` output does not reflect `sanitize_multi_select()` transformations — dry-run reads raw Claude-extracted fields before the live-write branch's sanitization pass. Confirm sanitizer behavior via unit test before trusting a dry-run for edge-case values.
