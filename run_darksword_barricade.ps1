# DARKSWORD — Barricade Cyber Automated Run
# Invoked by Windows Task Scheduler to run the Barricade Cyber pipeline.
...
$LogFile   = Join-Path $ScriptDir "darksword_barricade_$(Get-Date -Format 'yyyy-MM-dd').log"
...
Write-Log "=== DARKSWORD Barricade auto-run starting ==="
...
& $Python "notion_logger_v7.py" --auto-barricade *>&1 | ForEach-Object {
    $_ | Out-File -FilePath $LogFile -Append -Encoding utf8NoBOM
    Write-Host $_
}

$ExitCode = $LASTEXITCODE
Write-Log "=== Barricade auto-run finished — exit code: $ExitCode ==="
